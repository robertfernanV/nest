...
function BabyDetails() {
  ...
  const baby = useSelector((state) =>
    state.children.data.find((child) => child.selected)
  );

  const [babySelected, setBabySelected] = useState(null);

  useEffect(() => {
    setBabySelected(baby);
  }, [baby]);
  ...
  return (
    <React.Fragment>
      ...
      {babySelected !== null ? (
        <>
          <BabyCard
            familyId={babySelected.familyId}
            Id={babySelected.childrenId}
            age={babySelected.age}
            image={babySelected.image}
            memberName={babySelected.name}
            key={babySelected.childrenId}
          />
